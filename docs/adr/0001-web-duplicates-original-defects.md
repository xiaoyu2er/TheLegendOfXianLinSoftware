...
标注「待签」的那十行是 xl-03x.4 补登的草稿：**事实与理由都抄自代码里本来就有的注释**，
签字（认不认这个取舍）要人来，交主干核。⚠️ 这十行的「判据」一格是**按已有测试的文件名 / 用例名指过去的**，
...
| `start-exit-disabled` | **（待签 · xl-03x.4 补登）**标题「结」是 `System.exit(0)`（`src/start/StartPanel.java:233`） | 按钮画出来、**禁用**，带一句 `disabledReason` | 浏览器里没有对应物（`window.close()` 对地址栏进来的页面一声不吭）；xl-u23 量过三条路之后的定案（用户 2026-09-08 裁定）。⚠️ **这一行会被 `xl-03x.12` 改写**：M8 规格定的是「画出来、点了什么都不发生」，与菜单「确认离开」合成一行重签 | `start/buttons.test.ts` |
...
**10 行新登记**，落在 **12 处**代码位置（表里「待签」那十行；其中「面板动画线程」一行
...
- **没有行为差别的「没抄」**：`showDrug()` 死代码（`menu/drugPanel.ts`，有判据守零调用者）、
  ...
  （`menu/render/drawList.ts`，刷新已经在 `drinkDrug` 里）。原版做的事 web 端一件没少，只是换了地方做。
- **欠账，不是取舍**（有票、将来要做成与原版一致）：「起」不重建结局面板那条线程
  （`game/session.ts` 的 `Session.end`，M8 规格划进跨面板那一族）、菜单松手落在下一帧时被丢
