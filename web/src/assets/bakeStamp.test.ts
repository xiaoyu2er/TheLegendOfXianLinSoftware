...
describe('烘焙指纹', () => {
   ...
   *
   * **这一族对账在这里停（xl-cpv 的结论，不是没做完）。** 绕法写不完，而它们漏掉的风险很窄：
   * 登记行的挑法与烘焙器的实际挑法只要在**今天磁盘上的任何一个文件**上不一致，上面两条数据用例
   * 当场就红，这与烘焙器那道过滤写在哪儿无关：两条用例比的是登记的挑法与名单这个结果。实测只在装备
   * 行上做过（登记改成全收 → 「每一个文件都在输入名单里」红；登记排掉 `头/` → 「被某一行认领」红），
   * 商店行（烘焙器在循环里 `continue`）是按同一理推的，没篡改过。「被某一行认领」有个前提：各行认领的
   * 文件互不相交 —— `SHOP_ROOT` 与装备、药品两行前缀重叠，今天靠 `shopAssetOwner` 把那两个子目录判
   * 'elsewhere' 才不相交；前提破了，还剩那一行自己的逐目录用例。
   * 哑掉要四件事同时成立：烘焙器改了挑法且写在调用链外、登记没跟、两边只在**将来才出现**的一类文件上
   * 不一致且登记更窄、然后真有那样一个文件进来而没重烘。读数（截至 19ed2097，2026-09-13）：烘焙器
   * 诞生（0a0fba2f）以来，十个登记目录里新增过文件的只有 `tools/traces/out`（59 个）与 `src`（1 个，
   * `fix(diag)`）；前者的过滤就在调用链上，由上面的对账逐字核（xl-de7 篡改 T2：过滤改成 `.json` 就红）。
   * 装备行实测一例「将来的文件」另有人接：登记排掉 `*.new.png` 再放一张 `头/x.new.png`，这里照绿，
   * `equipDraw.test.ts` 与 `resolve.test.ts` 拿磁盘自己当分母各红一条。按编号 `existsSync` 那一类今天
   * 没有：那十几处循环的上界都是常量（`HEAD_COUNT`、`BG_COUNT`、`spec.count` 之类），在闭包里，改了就
   * 红；多出来的文件它们根本不去读。再往下做数据流跟踪，据判断复杂度与这一整段相当（没量过），换来的是
   * 四件事同时成立那一种情况。
   */
