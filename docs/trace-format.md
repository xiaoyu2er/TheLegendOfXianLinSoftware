...
`setup.levelUps`（可选，xl-03x.17）是 `{"zhang"|"lu"|"wen": 次数}`：开局对那个人调
原版自己的 `levelUp()` 那么多次（在 `new MenuPanel()` 与铺背包之后、`fullHeal` 之前），
调完核等级真的涨了那么多。它存在是因为菜单剧本原先让不了等级离开出厂值，奇术页的技能
格数于是永远是那三个 static 的初值。走 `levelUp()` 而不是直接写 `skillNumber`：直接写的
...
| `question-memory` | `大活.txt` → `大地图.txt` → `大活.txt` | **答过的题记得住**（xl-yg6.7）：答掉第一道题、出门进大地图、再原路走回来 —— 场景整个被 `initiation` 重建，而 `SelectEvent` 的构造函数按 `fileName` 在 `mapName` 里认领回**同一个** `haveAnswered`，于是回来之后 `answered[0]` 仍是真、走到同一个 NPC 面前弹出来的是**第二道**题（`questionNo` 1）。它同时给那条已登记的原版怪癖取到读数：在大地图上那半段，那个场景一道题都没有，`answered` 是 `[]`、`sceneNo` 停在初值 0，而 `recorder` 里已经躺着大活的记录 —— 「0 指着别人」 |
...
| `battle-victory` | 战斗（`driver` = `battle`） | **打赢之后的结算**（xl-rh9.13，837 步；xl-03x.14 加了一步 `debugKill` 之后 158 步，原先那版见下一行）：与 `battle-em3-box` 同一行 Fight 数据（`脚本20.txt` 第 3 行），而**陆雪琪压到 1 级** —— 这一场 1190 点经验对 10 级的张小凡与文敏差得远（升一级要 14462），对 1 级的陆雪琪却够（700），于是结算的两条路在同一份真值里都走到：第一页三个人、第二页只有升了级的那一个、属性一项项滚上去、`timeCode` 数到 55 才 `switchTo("scene")`。它是 `victory.ts`（xl-rh9.5）的**第一份行为真值覆盖** |
| `battle-victory-normal` | 战斗（`driver` = `battle`） | **正常打死 → 结算 → 回场景**（xl-5zw，837 步）：xl-03x.14 给 `battle-victory` 加了一步 `debugKill`（缩到 158 步）之后，这条路在真值里零覆盖，于是把按 J 之前那一版换名原样恢复。重导之后与 `9415307c^` 那份旧真值逐拍比，共有字段零处不同（只多了 xl-b36 之后才有的 `music`）；与现在的 `battle-victory` 前 52 拍逐字相同、在按 J 那一拍分开 —— 两者的差异只在判胜那一刻的战斗状态（行动条清零、怪物的血、控制台开没开），结算那一段同一段代码 |
