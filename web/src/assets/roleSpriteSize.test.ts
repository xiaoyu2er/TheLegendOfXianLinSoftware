 ...
 *
 * 所以裁在烘焙器里做，这里核产物。**核的是产物不是源码**，于是它有一个必须
 * 说清楚的边界：改坏 `scripts/bake.ts` 之后**要重跑 `pnpm bake`**，这条才会红
 * —— 门禁里没有任何一条重烘资源。改了烘焙器不重烘就跑测试，拿到的是上一批产物
 * 的结论，而它和"改对了"长得一样。（实测：不裁的那批产物在这里报
 * `run 第 14 帧: expected { width: 42, height: 65 } to deeply equal
 * { width: 42, height: 64 }`。）
 *
 * 下面那个 `pngSize` 与烘焙器里的那个是逐字重复的，**故意不抽出去共用**：
 * 期望值这一侧只要与被测的那一侧共用代码，共用的那段错了两边就一起错，而
 * 测试照绿。真正的交叉验证在于两侧读的是**不同格式**——烘焙器读 PNG 决定裁到
 * 多少，这里读 WebP 看它实际烘成了多少。
 */
