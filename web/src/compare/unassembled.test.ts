...
describe('各驱动器的处境表', () => {
  ...
  it('M4 收口的四支都有真值，且取图页装不出来的只有这里签过的那几支', () => {
    ...
    //
    // 签过的一笔（xl-i06.6）：`saveload` —— 存读档面板的真值先行，取图页装配归
    // xl-i06.12。那张票接上了，这一笔删掉，登记回到空。
    //
    // 签的一笔（xl-czb.5）：`end` —— 结局面板的真值先行（M7 的 SPEC 拆的顺序是
    // 「第六支驱动器 + 真值剧本」先、面板与接线后），面板与取图页装配归 xl-czb.6。
    // 那张票接上之后这一笔要删；不删，这条照样红。
    expect(
      drivers.filter((d) => !isImplementedDriver(d)),
      '取图页装不出来的驱动器与这里签的那一笔对不上。多出来的是新加的真值、取图页还没装配；' +
        '少了的是装配已经做出来、签的那一笔该删了 —— ' +
