...
#         2 = 流水线自己没跑完 —— 也包括「表态与取图页的实现状况对不上」
#             （表说比得了而页面装不出、页面装得出而表还写 unassembled）：
#             那种情况下一条剧本都没比过，与「比过了但不符合预期」是两回事。
#
...
# 空 = 不传给导出器，让**剧本自报的密度**说了算（缺省仍是导出器里的 25）。
# 从前这里写死 25 并且总是传下去，等于命令行永远在压剧本 —— 一条 47 步、
# 意义全在逐帧上的菜单剧本于是只出 2 帧，却照样打印"比过了"（xl-6lo.3）。
every=""
names=()
