...
——两条中任意一条成立就该再拆。`xl-rh9.4` 原本要覆盖 22 个更新动作 × 4 份
真值 + 25 层绘制 + 三个菜单，判据有四类（状态字段 / 像素 / 装配表对撞 /
缺陷复刻），是上表任何一张的 5–10 倍。它后来拆成了 `.7` / `.8` / `.9`。
...
票明显超出这个量级，**报上来，别硬做**。一坨两千行的改动里指不出是哪一处让
真值对不上——那时候「做完了」和「做错了」又长得一样了。
