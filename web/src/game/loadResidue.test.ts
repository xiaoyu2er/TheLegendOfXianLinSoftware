 ...
 *   以及「起」之后 `currentScript` 也带进新局（xl-9rv 裁定不复刻，见最后一组）。
 *
...
describe('读档 → 回标题 → 起：钱、药、装备库存、答题记录带进新局', () => {
   ...
   * 全部原样带进新局。
   *
   * 这里只断言这四样。原版也带进来、而 web **故意**回开机值的那几样，xl-9rv 裁定沿用 xl-lly 的
   * 例外（ADR-0001 例外表 `new-game-resets-party`），判据在 `game/useGameNewGameAfterLoad.test.tsx`：
   * - **等级 / 血 / 经验**：原版带进来（11/10/11 级），web 的「起」重置队伍 —— xl-lly 的例外本身；
   * - **身上的装备**：与等级**绑在一起** —— 穿着的那件的四项加成算在属性上，而 `resetParty()` 把
   *   属性回到出厂值。只带装备不带属性，弃用那一下会把加成扣成负的，所以跟等级一起回；
   * - **剧情三元组 `currentScript`、`isLoad`**：原版带进来（`currentScript` 仍是存档0 的
   *   `43/42 比武第二阶段 脚本38.txt`，「新局」的剧情接着存档里的走）。队伍回了出厂、剧情却接着
   *   存档走，是一局拼起来的游戏，所以一起回。任务文本归同一类，判据也在 `useGameNewGameAfterLoad.test.tsx`。
   */
