...
1. **闸门现数**：xl-03x（open）下选进来的实现票是 xl-03x.2（open）到 xl-03x.18（open）共 17 张，恰好等于 SPEC 的上限 17；xl-03x.20（open）是 xl-03x.4（closed）做着做着逼出来的，按 SPEC 不占额度。这一趟新立的七张都没进这一轮，所以**没有超闸门**。
