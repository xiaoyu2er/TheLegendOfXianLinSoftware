...
export function StartPanelView({ view, handlers }: StartPanelViewProps) {
  ...
  const onMouseDown = (event: ReactMouseEvent<HTMLDivElement>) => {
    ...
    // 同一判法）。全松开那一下 JDK 本会重设目标并派松手，而原版收不到它 —— macOS 上（CGEvent 合成的序列，「窗口外」那一下按在另一个 app 的窗口上）它根本到不了 Java 窗口（xl-zs6 实测；复跑 `tools/mouse-dispatch-probe.sh`，xl-sij）。换成同一个 app 的另一块窗口（xl-g9w，`--where same-app-window`）或者露出来的桌面（xl-23v，`--where desktop`），对原版那块窗口来说读数逐字相同。
