...
export interface StageProps {
  ...
  overlay?: ReactNode
  /**
   * 放进渲染器宿主里的东西（xl-rh9.17）。
   *
   * 一个面板一张画布，而**同一时刻只显示一张**：两个 Pixi `Application` 各往
   * 宿主里塞一张 canvas，`.stage-canvas` 又是 `width/height: 100%`，两张一起
   * 显示就会上下摞着 —— 取图页那一侧撞过这个（`replay/main.ts` 的 `hostFor`，
   * 症状是截图永远截到第一张）。所以调用方给的是几个各自 100% 的子容器，
   * 用 `display` 切。给了它就不再用 `hostRef`。
   */
 ...
 */
export function Stage({ scalingMode, hostRef, overlay, hostContent }: StageProps) {
