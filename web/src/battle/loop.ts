 ...
 * 游戏本体（`game/useGame.ts`）今天只跑场景，战斗还没接进去 —— 结算与回地图
 * 已经由 xl-rh9.5 做完（`battle/victory.ts`，切面板落在 `world.exitPanel`），
 * 欠的是把战斗面板挂进游戏本体那一步（SPEC xl-rh9 的最后一条验收）。也就是说
 * **判据成立，被判的对象还没接上** —— 上面那条不变量验的是这个推进器，
 * 不是"真在游戏里切一次标签页"。
 *
