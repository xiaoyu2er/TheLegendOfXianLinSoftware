...
| 3. 对照表每一行各有一条会红的判据 | **一半成立**：判「能」的 49 行，每一行的依据列都引到了一份磁盘上存在的测试文件或剧本（机器核）；「不能 / 不对」的 13 行，会红的是认领关系本身（认领的票关了而这一行没改 → 判据 1 红）。⚠️ **「引到的那份测试在这一行坏掉时真的会红」没有逐行篡改验证过** —— 验过的只有 M8 各票自己篡改过的那些行 | `playerCoverage.test.ts`：「能」行没引到存在的测试或剧本、引的测试文件不存在 —— 都红 |
...
| 每张 open 票恰好一个身份标签 | **成立**（本票与 M8 的 SPEC 关掉之后）：open 64 张 = 登记 35 · 下一轮 11 · 没人认领 18 | `issueIdentity.test.ts`：少一个、多一个、认不出、与快照不是同一次导出 —— 都红 |
