...
⚠️ 这个字段**会改剧本回显**（导出器把它写进 trace 头），所以加它必然要重导那两份
菜单真值 —— 那是预期内的 diff，diff 只有 `"every": 1` 这一行新增，`tickCount`、
每一步的每个字段逐字节不变；其余 19 份真值零变化。加密度字段以外的剧本改动
（比如给某份场景剧本补一个 `every`）也是同样的账，先想清楚要不要付。
