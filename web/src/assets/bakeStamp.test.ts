...
describe('烘焙指纹', () => {
   ...
   * 当场就红，不管烘焙器那道过滤写在哪儿（实测：装备行登记改成全收 → 「每一个文件都在输入名单里」红；
   * 登记排掉 `头/` → 「被某一行认领」红）。哑掉要四件事同时成立：烘焙器改了挑法且写在调用链外、
   * 登记没跟、两边只在**将来才出现**的一类文件上不一致且登记更窄、然后真有那样一个文件进来而没重烘。
   * 烘焙器诞生（0a0fba2f）以来十个登记目录里新增过文件的只有 `tools/traces/out`（59 个）与 `src`
   * （1 个，`fix(diag)`），而前者的过滤就在调用链上、逐字核着。装备行那种「将来的文件」实测也另有人
   * 接：登记排掉 `*.new.png` 再放一张 `头/x.new.png`，这里照绿，`equipDraw.test.ts` 与 `resolve.test.ts`
   * 拿磁盘自己当分母各红一条。再往下做数据流跟踪，复杂度是这一整段的量级，换来的是四件事的合取。
   */
