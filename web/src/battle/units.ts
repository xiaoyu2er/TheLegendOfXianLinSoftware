 ...
 * case；xl-3hn 之后，**全库脚本用到的每一只**都在这里（各连同一份原样照搬
 * 那一行 Fight 数据的战斗真值），没抄的只剩脚本里一次都没出现的那几只。
 * 行数**不写进任何断言**；「全库用到的一只不缺」由 `playthrough.test.ts`
 * 的 `monsterGap` 现数守着。查不到时的表现是**当场抛并点名**，不是"用一份
 * 默认值悄悄打下去"。
 */
...
export const ENEMIES: Readonly<Record<string, EnemySpec>> = {
  ...
  // 以下三只由 xl-rh9.14 那五份真值盖住（技能与秘术）。挑它们的理由写在各自
  // 剧本的 description 里：大刀全表最慢、蒙面怪人皮厚而手轻、物理阁大弟子
  // skillNum=1（每一发都是普攻，怒气攒得稳）。
  大刀: {
    length: 4,
    beAttackedFrames: 6,
    speed: 4,
    ...
    thing: '蓝格怪衣/2',
    skillNum: 2,
    beAttackedOffsetX: -70,
    beAttackedOffsetY: -60,
    skill: {
      name: '怪物/大刀攻击',
      length: 26,
      offsetX: -70,
  ...
  // 以下由 xl-3hn 那几份主线剧情战的真值盖住（`battle-script*`，各照搬原脚本
  // Fight 那一行）。
  武林高手1: {
    length: 4,
    beAttackedFrames: 6,
