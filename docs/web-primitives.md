...
| `web/src/start/StartPanel.tsx:256,257,262,263` | 事件 / 全局 | 对应 `src/start/StartPanel.java:192` | `mouseReleased`：面板上按下那一刻把松手挂到 window 上（Swing 的 mouse grab），落点不在任何一颗按钮上照样送 —— `setButton()` 不看坐标，拖出框松手照样触发；`buttons` 为 0 才解除，丢了 `mouseup`（窗口外 / 禁用的「结」上；「结」那半无头 Chrome 153（CDP 派输入）量过、窗口外没量，xl-qzx）由下一次没按键的移动、或除自己之外没按着别的键的新按下补上（xl-4zo）；松手先记自绘鼠标坐标，grab 期间 window 上的 `mousemove` 是 `mouseDragged`（`:211`），也记 —— 舞台外照记、不裁（JDK 17 `LightweightDispatcher` 照样派给 grab，坐标只减面板偏移，xl-40m；越界坐标 macOS 实测到了，`DRAGGED` 一路 `src=start.StartPanel`、x 到 1254，xl-bg3）；grab 期间在**舞台外**按下的第二个键，按下与松手原版一下都收不到 —— 那一下按在别的窗口上，归那个窗口（macOS 实测三轮，CGEvent 合成的序列，同一轮里拖回窗口内再按右键读到 `PRESSED btn=3 src=start.StartPanel`；票面照 JDK 推的「原版派 mousePressed」不成立，xl-bg3）：按下本来就不收（window 上没挂 `mousedown`），松手靠「这个 grab 收下过按下的那几只键」那张位图认出来，不送 `release`、也不记坐标；起 grab 那只键的松手照送（`isMouseGrab` 把本键异或**回去**，读的是按下之前的状态，最后一只键松开时它仍为真、目标不重设）；`start/StartPanel.test.tsx` |
| `web/src/start/StartPanel.tsx:298,393` | 无障碍 / 元素 | 已登记 `ADR-0001#screen-reader-text` | 按钮 `aria-label`、「关于我们」的 `alt`（**xl-03x.22 补登**） |
| `web/src/start/StartPanel.tsx:300` | 无障碍 | 已登记 `ADR-0001#start-exit-disabled` | 「结」禁用的理由 |
| `web/src/start/StartPanel.tsx:310,311,312` | 事件 | 对应 `src/start/StartPanel.java:202` | `mouseMoved`；按着键的移动 / 移入 / 移出是 `mouseDragged`（`:211`，只记坐标、不跑 `isMoveIn`），不碰悬停（xl-vi8；移出 xl-4zo） |
| `web/src/start/StartPanel.tsx:316` | 事件 | 已登记 `ADR-0001#start-focus-hover` | 鼠标按下不给焦点：焦点 = 悬停那条加法只留给键盘，不然它会把 `isPressedButton` 刚停掉的高亮又转起来（xl-4zo）；无头 Chrome 153（CDP 派输入）量过：按过之后 Tab + 回车 / 空格照按得动（xl-qzx） |
| `web/src/start/StartPanel.tsx:321,322` | 事件 | 已登记 `ADR-0001#start-focus-hover` | 键盘焦点 = 悬停 |
| `web/src/start/StartPanel.tsx:325` | 事件 | 已登记 `ADR-0001#start-button-hitbox-dom` | 只收键盘的激活（`detail` 为 0）：原版标题页没有键盘输入，这一条是「真 `<button>`、回车按得动」那一半（真浏览器里曾经按不动，`xl-fqm` 修了）。鼠标不走它 —— 按下 / 松手分两下送（xl-4zo） |
| `web/src/start/StartPanel.tsx:337,346,361,367,377,403,412,428` | 无障碍 / 元素 | 工程 | 装饰图的 `alt=""`（读屏跳过） |
| `web/src/start/StartPanel.tsx:353` | 元素 / 事件 | 对应 `src/start/StartPanel.java:185` | `mousePressed`：面板上任何一处按下，先记自绘鼠标坐标（头一句 `currentX = e.getX()`，xl-40m）（落在空处按 `null`，只把图换回常态）；grab 不在这块面板上而这一下之外还按着别的键（舞台外按着键拖进来），`isMouseGrab` 为真、目标仍是 null，不收；收下的每一只键都记进那张位图，上面那一行的松手照它判该不该送（xl-bg3）（xl-m9q；全松开那一下 JDK 本会派松手，而 macOS 上实测它根本到不了 Java 窗口、原版也不收 —— **没有残余差异**。读数取自 CGEvent 合成的序列，窗口外那一下按在另一个 app 的窗口上，xl-zs6；复跑 `tools/mouse-dispatch-probe.sh`，xl-sij）；同一行的 `onMouseMove` 是自绘鼠标的坐标（`mouseMoved` 头一句 `currentX = ex.getX()`；按着键是 `mouseDragged`（`:211`），归挂到 window 上的那个 `mousemove`（`:213`），只在 grab 挂在这块面板上时才记 —— 舞台外按下再拖进来，原版目标是 null、一次都不派（xl-b98））；同一行的 `onDragStart` 取消面板上图片的原生拖放 —— 不取消的话真 Chrome 里在背景上按下一拖就 `pointercancel`，之后的 `mouseDragged` / `mouseReleased` 一个都收不到（xl-qzx） |
