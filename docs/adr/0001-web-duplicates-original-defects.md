...
**读数**：137 行命中、48 个单元；键位 10 行（原版 9 个键、web 12 个键名）。**新登 5 行**（上表 `shift-also-runs` /
...
- **不经过任何浏览器 API 的加法。** 状态层里多一条纯函数分支（例如「跳过逐字打印」在 `state/dialogue.ts` 的那一半）
  一个平台 API 都不调，这里只看得见它挂在键盘上的那一头；只改画面的 CSS（颜色、布局、`pointer-events`）也不在分母里；
