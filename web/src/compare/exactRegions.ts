 ...
 * ## 「相等」是**容差之内**相等，不是字面零差
 *
 * 容差沿用整屏那条口径（`DEFAULT_TOLERANCE`，跑的是 8），与 `regions.ts` 的硬比区
 * 同一句话：「每一个像素都必须在容差内相等，不是超过阈值才算偏，是一个都不许
 * 有」。**这不是给它留的余地，是这一层做不到字面零差**：同一批帧上把容差调到 0
 * 现数，15 帧 22620 个像素里有 **7526 个**通道差 1..8（2026-09-08 实测，最大的
 * 一帧是 `battle-yu-skills` 的 t=800，2880 个里 1403 个）—— 提示图带透明，压在
 * 有损重编码过的背景动画上，两端的合成取整差一档。
 *
 * 记这个数是为了**它自己也是一条判据**：这一块要是哪天真能字面零差了，说明合成
 * 那一路也对齐了，那时再收紧容差；而在那之前，把名字读成"零差"会让人以为守得
 * 比实际严。
 *
 * ## 两个分母
 *
