 ...
 * 全都现扫自磁盘，量的是「名单和产物一致吗」，所以一个**改了名单又重烘过**的
 * 错边界能整批骗过它们 —— 两边一起错，零个差异。
 *
 * 2026-09-08 实测，把 `MENU_SKELETON_TOP_DIRS` 去掉 `物品`（也就是砍掉从句二）
 * 再跑一次 `pnpm bake`，产物随之变成 43 / 146，**这一档 15 条里 13 条照绿**：
 * 双向盖满、分母相加、按需没进主包、产物路径不撞车，全部通过。只有两条红 ——
 * 「边界与原版的第一帧对得上」和「只放行顶层目录逐字相等的那些」。
 *
 * ⚠️ **而 43 / 146 正是这张票初版交出去的状态。** 它当时 14 条全绿，是
 * `/code-review` 的 Spec 轴靠读注释发现边界只做了半句判据的 —— 没有任何一条
 * 自动检查会响。这一条就是补上的那个响法：少了它，边界画错在这里**只由几个
 * 手写的例子兜着**。
 */
