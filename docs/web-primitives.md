...
| `web/src/start/StartPanel.tsx:328` | 元素 / 事件 | 对应 `src/start/StartPanel.java:185` | `mousePressed`：面板上任何一处按下，先记自绘鼠标坐标（头一句 `currentX = e.getX()`，xl-40m）（落在空处按 `null`，只把图换回常态）；grab 不在这块面板上而这一下之外还按着别的键（舞台外按着键拖进来），`isMouseGrab` 为真、目标仍是 null，不收；收下的每一只键都记进那张位图，上面那一行的松手照它判该不该送（xl-bg3）（xl-m9q；全松开那一下 JDK 本会派松手，而 macOS 上实测它根本到不了 Java 窗口、原版也不收 —— **没有残余差异**。读数取自 CGEvent 合成的序列，窗口外那一下按在另一个 app 的窗口上，xl-zs6）；同一行的 `onMouseMove` 是自绘鼠标的坐标（`mouseMoved` 头一句 `currentX = ex.getX()`；按着键是 `mouseDragged`（`:211`），归挂到 window 上的那个 `mousemove`（`:213`），只在 grab 挂在这块面板上时才记 —— 舞台外按下再拖进来，原版目标是 null、一次都不派（xl-b98））；同一行的 `onDragStart` 取消面板上图片的原生拖放 —— 不取消的话真 Chrome 里在背景上按下一拖就 `pointercancel`，之后的 `mouseDragged` / `mouseReleased` 一个都收不到（xl-qzx） |
