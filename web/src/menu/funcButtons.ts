 ...
 *
 * 存档 / 提取进存读档面板（xl-i06.9，经 `saveLoadRequest` 交给会话）。退出那一组
 * 的两颗各有去处：「重新开始」**照画、照响应三态、点得响**，后半段（回标题）
 * 归 xl-03x.11；「确认离开」是 `System.exit(0)`，浏览器里没有对应物，**画出来、
 * 禁用**（`FUNC_DISABLED`，xl-03x.12）。禁用不碰 `isDraw` —— 真值 `func.drawn`
 * 记的正是它，所以那一列照旧；少掉的只是三态贴图里的「待点」与「按下」两态。
 *
