...
| 2. 全量跨端逐帧比对 N/N 符合预期 | ⚠️ **这一条的读数已经过期，不是当前状态**：2026-09-11（`f31e4580`）那次跑的是 **45/45 条剧本符合预期**（合计 1818 帧；退出码 0），而当天 `tools/traces/scripts/` 正好是 45 份 —— 那句「N = 现数 45」把一个**当时**的读数写成了一个看起来会自己更新的现数。**现读 63 份**（2026-09-17，`ls tools/traces/scripts/*.json | wc -l`），此后新增的 **18 份从没跑过全量逐帧比对**：battle-drugs / battle-mouse / battle-script3 / 6 / 12 / 15 / 17 / 25 / 31 / 38-2 / 38-3 / 39 / battle-victory-normal（13 份）与 start-about / start-about-again / start-drag-out / start-hover-end / start-newgame（5 份）。下面这段缺口区的记账同属 2026-09-11 那一趟。⚠️ **别把「45/45」读成「全都比过了」** —— 要当前读数得重跑 `tools/compare-frames.sh`（xl-dph（open））。以下是 2026-09-11 那趟的原文：　缺口区复核：报告里 144 条缺口区记账、49 个不同的区名，**没有一个区的最差帧是 0** —— 没有「账还完了而区还留着」的；比对器本身对「某区一帧都不差」双向红，所以这一条也是它守着的。每个区的成因写在 `web/src/compare/expected.ts` 那个区的 `why` / `issue` 上（类型强制，缺了编译不过）；这一轮之内被删掉的区只有旁白第 599 行那一个（xl-03x.15（closed））。账本对撞 18 条剧本逐帧相等。⚠️ 同一天第一次全量跑时，`maze-treasure` 取到第 4 帧后卡死 9 分钟（进程全部 0% CPU）；单独重跑那一条通过、第二次全量也通过 —— 记为偶发，没有查到原因 | `tools/compare-frames.sh` 退出码 |
...
| 每张 open 票恰好一个身份标签 | **成立**，现读（2026-09-17）：**open 50 张，每张恰好一个身份 —— 登记 35 · 下一轮 11 · 没人认领 4**（`tools/issue-snapshot/identity.json` 现数；合计 50 = open 票数，零个与多个身份的都没有）。⚠️ 这一格的**读数会随开关票变动，而判据只验「恰好一个」、不验这几个数**，所以它跟下面那些「怎么红的」一样是**某一时刻的读数**，不是一个会自己更新的现数 —— 第 17 行那个「N = 现数 45」就是这么过期的（xl-dph（open））。M8 收口时的读数是 open 64 张、登记 35 · 下一轮 11 · 没人认领 18（当时没人认领里含 blocked 的 xl-u7b（closed）——那张票 2026-09-17 做完关掉了）。关票前快照 open 66 张、64 张有身份，**缺的两张恰好是本票与 M8 的 SPEC**，判据当场红在这两张上 —— 那是「少一个」在真快照上的读数。「多一个」的真快照读数：给 xl-y11（closed）临时再贴一个身份、重导，判据红「贴了 2 个身份」；撕掉重导后快照逐字节回到原样 | `issueIdentity.test.ts`：少一个、多一个、认不出、与快照不是同一次导出 —— 都红 |
