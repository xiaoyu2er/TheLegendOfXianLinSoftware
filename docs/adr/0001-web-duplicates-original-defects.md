...
| `start-focus-hover` | **（xl-03x.4 补登 · xl-03x.19 收口签）****原版没有键盘焦点**：标题四颗按钮只认鼠标坐标 | **加了**：Tab 到一颗按钮上等于鼠标移进来（换图 + 那圈高亮转起来），离开等于移出 | 无障碍。只补 CSS 的话 Tab 过来的人看到一颗半死的按钮 | `start/StartPanel.test.tsx`「键盘 Tab 过来也换图、也转高亮」；**鼠标按过之后键盘照按得动**由 xl-qzx 在无头 Chrome 153 里量过（按下的 `preventDefault` 生效、焦点留在 body，Tab + 回车 / 空格照样触发）|
...
| `drag-not-grabbed-to-hidden-panel` | **（xl-bwl 补登 · 主干签；xl-4xi 扩写）**Swing 的 `LightweightDispatcher` 把 `MOUSE_DRAGGED`、以及 grab 期间的 `MOUSE_PRESSED`（和弦里的第二次按下）都按 grab 派：在一块面板上按下、面板随即被切走，**藏着的那块照样收拖动** | **拖动与和弦里的第二次按下都照当前面板过滤**（`game/useGame.ts` 的 `routeByGrab`：只有松手按 grab 派；和弦的按下仍然计数，所以松手照样送给藏着的那块 —— xl-4xi）。松手、以及拖到宿主之外的那一段（xl-b28 / xl-bwl）都是照 grab 走的，唯独「面板已经被切走」这一种不送 | 四块宿主逐块查过为什么今天看不出来：菜单、存读档见 `game/session.ts` 那两段（悬停贴图下次开菜单头一下移动全刷 / 只记坐标、松手重写）；店换面板只有「返回游戏」一条而它在 `mouseReleased` 里、键也到不了店，按着的时候店不会被切走；战斗那条**走得到**（战斗线程自己结束、玩家还按着），⚠️ 但「那几下拖动留不留得到下一场」是**未验证的推理**，没有量过。四块的理由写在 `routeByGrab` 的头注里 | **暂无会红的判据**：这一行登记的是「不送」，而送不送今天在四块上都看不出后果；战斗那一块要有判据，得先把上面那条未验证的推理量出来 |
| `victory-late-rescene` | **（xl-sn2 补登 · 主干签）**没升级的胜利回地图之后，`VictoryReminder.update()` 仍在原版战斗线程上跑，`timeCode` 从 15 加到 55 时（40 拍、约 4 秒）**再 `switchTo("scene")` 一次**：人在菜单里会被拽回场景，人在场景里场景曲再 `play` 一次、从文件头开（`BattleIdleProbe` 的 rescene 模式实测；⚠️ 听感未量 —— 跑的时候 CAN_PLAY_BGM=NO；商店 / 存档页 / 标题页是同一句 `switchTo`，读代码得出、未跑）。4 秒内开了新一场则不发生（实测：`timeCode` 归 0，80 拍里 tap 一直是 1）；升级那一支本来就在 55 切，不会再等于 55 | **不复刻**：web 在出口那一拍把战斗世界置 `null`（`game/session.ts`），战斗循环不再推，第二次切换不会发生 | 原版自己两支不一致（升级的那支不会这样），而复刻它要让战斗世界在出口之后还活着、还能从战斗线程去切别的面板 —— 为一个只在「没升级 + 4 秒内没开新仗」时出现的行为，换掉出口那一刀，不划算。xl-03x.23 量出机制、xl-sn2 量出后果，两处读数都在票里 | **暂无会红的判据**：后果是跨面板的，真值要一条剧本同时推战斗 / 场景 / 菜单三块面板 —— 押在 `xl-x0t`。rescene 是探针不是可复现导出（菜单那四条线程自由奔跑，没有 `--check`）。⚠️ 主干只复现了负对照（升级那一支 60 拍 BGM play 1 次、tap 恒 1），正面那一例的 nolevel 变体没入库 |
...
- **未核 → 已量（xl-03x.23，2026-09-13）**：战斗线程在战斗之外的那一段（`BattlePanel.run()`
  开机起、关机停，战斗结束后仍在调 `launchAttack.check()` / `gameOver.update()` 等）改不改得到
  下一场的状态 —— 探针 `BattleIdleProbe` 六种收场各空推 200 拍逐字段转储：**动的只有帧相位与
  两个自己的计数**，英雄 static、实例、battleState、钱、背包一个字段都没动；接着开下一场推 150 拍
  也只差帧相位。**一处例外**：没升级的胜利回地图之后 `timeCode` 照加，约 40 拍（4 秒）时
  `VictoryReminder.java:435` **又 `switchTo("scene")` 一次**；web 在出口那一拍丢掉战斗世界，
  不复刻。后果（人在菜单/店里会被拽回场景、在场景里场景曲从头重放）是读代码得出的、**没跑过**，
  归 `xl-sn2`。**xl-sn2 把后果也量出来了，主干据此签进上面的例外表**（`victory-late-rescene`，2026-09-13）：
  当初这里写「裁之前它是一张写明欠什么的票」，那张票已经裁完 —— 登记、不复刻。
