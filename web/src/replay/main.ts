...
const api: ReplayApi = {
  ...
  async seek(t: number) {
    ...
    renderer.showRole(breakRender(world.role, t))
    await twoFrames()
 ...
 */
function breakRender(role: World['role'], t: number): World['role'] {
  const b = window.__xlBreak
  if (!b || t < b.fromTick) return role
  return { ...role, px: role.px + b.heroDx }
}
