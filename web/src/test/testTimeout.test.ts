...
describe('用例超时', () => {
   ...
   * 它要是哪天变成 `undefined` 或者恒返回全局那个值，上面那条就成了摆设 ——
   * 而摆设和判据长得一样。这里给一条用例单独传一个第三参数，读回来必须正好
   * 是它。`1234` 与 `FLOOR_MS` 无关也不相等，所以这条通过就等于「读数跟着
   * 每条用例自己的配置走」。
   *
   * 只断言这一件事。**曾经在这后面跟过一句 `toBeLessThan(FLOOR_MS)`，那是
   * 恒真的**：上一行钉死了 1234，1234 < 30000 再也不可能红。恒真的断言看起来
   * 像多了一道保险，实际是多了一行永远不会说话的代码。
   */
