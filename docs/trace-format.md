...
| `battle` | `BattlePanel.run()` 的一次循环体 + 一次 `paint()` | `battle-min` `battle-em3-box` `battle-menus` `battle-victory` `battle-defeat-scene` `battle-defeat-start` `battle-defeat-slot2` | `BattleDriver.java` |
...
**跨端比对这条剧本要 `--every 1`。** `tools/compare-frames.sh` 缺省 25 个 tick 取
一帧，`menu-magic` 47 步于是只出 2 帧 —— 而这条剧本的全部意义就在逐帧上，2 帧几乎
什么都没采到。M3 把菜单接上线时记得改采样密度；这一条今天还是 `unassembled`，
所以现在只是记在这里，不是已经踩到的坑。
