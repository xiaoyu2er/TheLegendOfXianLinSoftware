...
| `menu-task` | 菜单（`driver` = `menu`，`setup.scene` = `脚本1.txt`） | 顶栏「当前任务:」走 if 那一支（xl-03x.10）：驱动器先 `new Reader("脚本1.txt")`，`task` 8 步全是「快去大活看演出吧」，四页各翻一次、每页顶栏都画。另外五条菜单剧本 `task` 恒为 `null` —— 它们从不进场景 |
