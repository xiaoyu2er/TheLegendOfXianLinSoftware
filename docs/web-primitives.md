...
读数（2026-09-13）：共 140 行命中、53 个单元（对应 43 · 已登记 38 · 工程 59 · 未核 0）
...
| 调用点 | 类 | 判定 | 证据 |
|---|---|---|---|
| `web/src/app/App.tsx:74` | 元素 / 无障碍 | 已登记 `ADR-0001#saveload-notices` | 存读档面板上那几行 `role="status"` |
| `web/src/app/App.tsx:493,494,495,499,500,501` | 事件 / 全局 | 对应 `src/menu/MenuPanel.java:100` | `mouseReleased`：松手按「按下那一刻」的组件派（Swing 的 mouse grab）。菜单是 xl-z4f，商店与存读档两块宿主 xl-o9z 收拢进同一个 `grabRelease`（归谁在 `useGame.routeByGrab` 定），战斗画布 xl-qqw 接进来（`BattlePanel.java:331`，拖出画布再松手照样触发）；按下到松手之间 window 上同时挂 `mousemove`，拖出宿主的 `mouseDragged` 也按 grab 派（xl-b28；对应 `BattlePanel.java:369`、`MenuPanel.java:116`、`LoadAndSavePanel.java:188`、`ShopPanel.java:200`、`EquipmentShopPanel.java:276`），grab 期间别的宿主不收移动；`mousemove` 与 `mousedown` 挂在捕获阶段，窗口外松了手（window 收不到 mouseup，原版照样收到）之后头一下没按键的移动或新按下当场补上松手（xl-bwl，坐标是见到的那一刻）；和弦（按着一个键再按另一个）照 `Container.java` 的 `isMouseGrab`：第二次按下也归 grab，每一次松手都派给 grab，所有键都松开才解除，窗口外松开按按下次数补松手（xl-4xi）；grab 开始之前舞台外就按着别的键，再在宿主上按下：`isMouseGrab` 为真、`mouseEventTarget` 不重设，目标是最后一个不在 grab 里的事件定的（不收鼠标的面板上那一下按下 / 无键离开窗口那一下 MOUSE_EXITED，都是 null），于是这一下按下不送、不起 grab，它与别的键的松手一个都不送（xl-2yh；macOS 实测，CGEvent 合成的序列、窗口外那一下按在另一个 app 的窗口上：那只键**在**回窗口后的 `getModifiersEx` 里，`mex=0x1400`，而它自己那一下松手到不了窗口，xl-zs6；复跑 `tools/mouse-dispatch-probe.sh`，xl-sij）；没有 grab 时按着键移到宿主上：MOUSE_DRAGGED 的 `isMouseGrab` 按着键恒为真、从不重设目标，目标同样是 null，一个 `mouseDragged` 都不派，全松开之后的移动照常（xl-5ee）；grab 期间在**舞台外**按下的第二个键，按下与松手原版一下都收不到 —— 那一下按在别的窗口上、归那个窗口（xl-bg3 在标题页上的 macOS 实测三轮，复跑 `tools/mouse-dispatch-probe.sh`；⭐ 三支落点都量过了，结论都相同：换成**同一个 app 的另一块窗口**（xl-g9w，`--where same-app-window`）或**露出来的桌面**（xl-23v，`--where desktop`），对原版那块窗口来说读数**逐字相同**（桌面那一支唯一的差异是 D2 松左那行的 `xy`，它按构造就是落点换算到面板内坐标，脚本单独摘出来报）；原生全屏构造上量不了（推理）；⚠️ 这四块宿主没有各自复量过，「到不到得了这个窗口」由操作系统按窗口定、与当前显示哪一块 `CardLayout` 面板无关是**推理**）：按下按舞台外接矩形挡掉，松手靠「这个 grab 收下过按下的那几只键」那张位图挡掉，位图空着时窗口外那条补松手的通路也一次都不补（xl-df1）。**「宿主外」不等于「窗口外」**：`CardLayout.layoutContainer` 把当前那块面板铺满整个内容面板，所以舞台里、宿主外（overlay、露出来的另一块宿主）在原版仍是同一块面板，按下照送；拖动也过这张位图（xl-8eg）：**位图里还有键按着**就照送 —— 按着键拖出窗口操作系统照样送进来（xl-40m / xl-bg3 实测越界 `DRAGGED` 一路 `src=start.StartPanel`，x 到 1254）；**位图空了**（起 grab 那只键已松开、只剩舞台外按下被挡掉的那只还按着）一条都不送 —— 2026-09-16 macOS 实测三轮，那一段 **Java 侧零条事件**，连 `ENTERED` / `MOVED` 都没有，拖动归「起这次拖动的那只键按下时所在的那个窗口」（同轮两个正对照：起 grab 那只键按着拖出去、面板里按右再拖，两段都读到 `DRAGGED`，所以那个零不是探针瞎了；⚠️ 与同格上文同一句限定 —— 量的是**标题页**，这四块宿主没有各自复量过；读法见 `tools/mouse-dispatch/expected-events-D.txt`）。⚠️ 位图非空、而起 grab 那只键已松开那一支**没量过**，照 `isMouseGrab` 推着照送；`app/appMenu.test.tsx`、`app/appGrab.test.tsx` |
| `web/src/app/App.tsx:570` | 元素 | 工程 | 外壳 `div` |
| `web/src/app/App.tsx:586` | 事件 | 对应 `src/battle/BattlePanel.java:309` | 战斗画布的鼠标按下 |
| `web/src/app/App.tsx:587` | 事件 | 对应 `src/battle/BattlePanel.java:350` | 战斗画布的 `mouseMoved`；按住左键时按 `buttons` 分成 `mouseDragged`（`:369`，少一句 `enemySlector.checkMoveIn`）（xl-qqw）；`app/appGrab.test.tsx`、`battle/pointer.test.ts` |
| `web/src/app/App.tsx:594` | 事件 | 对应 `src/menu/MenuPanel.java:93` | 菜单 `mousePressed` |
| `web/src/app/App.tsx:595` | 事件 | 对应 `src/menu/MenuPanel.java:109` | 菜单 `mouseMoved`（与 `mouseDragged` 两支逐字相同） |
| `web/src/app/App.tsx:596` | 事件 | 已登记 `ADR-0001#list-clipped-with-scrollbar` | 滚轮：原版没有这种输入 |
| `web/src/app/App.tsx:597` | 无障碍 | 已登记 `ADR-0001#start-exit-disabled` | 天书页「确认离开」禁用的理由挂在宿主 `title` 上 |
| `web/src/app/App.tsx:604,605` | 事件 | 对应 `src/shop/ShopPanel.java:173` | 两家店的按下 / 移动（装备店是 `EquipmentShopPanel.java:249` 同形的一段）；按住任一键移动送 `drag`（`ShopPanel.java:200`，只记坐标不跑 `isMoveIn`，xl-bwl）。**松手不在宿主上**，走上面那一行的 `grabRelease`（xl-o9z） |
| `web/src/app/App.tsx:612,613` | 事件 | 对应 `src/start/LoadAndSavePanel.java:161` | 按 `buttons` 分开 `mouseDragged` 与 `mouseMoved`，任一键都算拖动（xl-bwl）；松手同样走 `grabRelease`（xl-o9z） |
| `web/src/app/App.tsx:629,638,643,648,653` | 元素 / 无障碍 | 已登记 `ADR-0001#loading-notices` | 「正在载入 …」与渲染失败那一行（**xl-03x.22 补登**） |
| `web/src/app/App.tsx:634` | 事件 | 工程 | `<StartPanel onNewGame onLoad>` 是组件 prop，不是浏览器事件 —— 正则的误报，如实记下 |
| `web/src/app/App.tsx:664,704,731,732` | 元素 / 无障碍 / 事件 | 已登记 `ADR-0001#toolbar-under-stage` | 工具栏、操作提示、放大方式切换（**xl-03x.22 补登**） |
| `web/src/app/App.tsx:666,676,680,682,690,694,697` | 元素 / 事件 | 工程 | 场景 / 商店两个选择器，只在开发模式或 `?dev` 下渲染（`app/devTools.ts`） |
| `web/src/app/App.tsx:723,725` | 事件 / 无障碍 | 已登记 `ADR-0001#stage-scales-to-window` | 全屏按钮 |
...
| `web/src/replay/main.ts:184,187,189,199,204,231,417,604,663,699,902,916,937,938,942,943,945,946` | 全局 / 图片 / 定时 / 事件 | 工程 | 逐帧比对的取图页，不进游戏 |
...
| `web/src/start/StartPanel.tsx:263,264,269,270` | 事件 / 全局 | 对应 `src/start/StartPanel.java:192` | `mouseReleased`：面板上按下那一刻把松手挂到 window 上（Swing 的 mouse grab），落点不在任何一颗按钮上照样送 —— `setButton()` 不看坐标，拖出框松手照样触发；`buttons` 为 0 才解除，丢了 `mouseup`（窗口外 / 禁用的「结」上；「结」那半无头 Chrome 153（CDP 派输入）量过、窗口外没量，xl-qzx）由下一次没按键的移动、或除自己之外没按着别的键的新按下补上（xl-4zo）；松手先记自绘鼠标坐标，grab 期间 window 上的 `mousemove` 是 `mouseDragged`（`:211`），也记 —— 舞台外照记、不裁（JDK 17 `LightweightDispatcher` 照样派给 grab，坐标只减面板偏移，xl-40m；越界坐标 macOS 实测到了，`DRAGGED` 一路 `src=start.StartPanel`、x 到 1254，xl-bg3）；grab 期间在**舞台外**按下的第二个键，按下与松手原版一下都收不到 —— 那一下按在别的窗口上，归那个窗口（macOS 实测三轮，CGEvent 合成的序列，同一轮里拖回窗口内再按右键读到 `PRESSED btn=3 src=start.StartPanel`；票面照 JDK 推的「原版派 mousePressed」不成立，xl-bg3）：按下本来就不收（window 上没挂 `mousedown`），松手靠「这个 grab 收下过按下的那几只键」那张位图认出来，不送 `release`、也不记坐标；起 grab 那只键的松手照送（`isMouseGrab` 把本键异或**回去**，读的是按下之前的状态，最后一只键松开时它仍为真、目标不重设）；`start/StartPanel.test.tsx` |
| `web/src/start/StartPanel.tsx:305,400` | 无障碍 / 元素 | 已登记 `ADR-0001#screen-reader-text` | 按钮 `aria-label`、「关于我们」的 `alt`（**xl-03x.22 补登**） |
| `web/src/start/StartPanel.tsx:307` | 无障碍 | 已登记 `ADR-0001#start-exit-disabled` | 「结」禁用的理由 |
| `web/src/start/StartPanel.tsx:317,318,319` | 事件 | 对应 `src/start/StartPanel.java:202` | `mouseMoved`；按着键的移动 / 移入 / 移出是 `mouseDragged`（`:211`，只记坐标、不跑 `isMoveIn`），不碰悬停（xl-vi8；移出 xl-4zo） |
| `web/src/start/StartPanel.tsx:323` | 事件 | 已登记 `ADR-0001#start-focus-hover` | 鼠标按下不给焦点：焦点 = 悬停那条加法只留给键盘，不然它会把 `isPressedButton` 刚停掉的高亮又转起来（xl-4zo）；无头 Chrome 153（CDP 派输入）量过：按过之后 Tab + 回车 / 空格照按得动（xl-qzx） |
| `web/src/start/StartPanel.tsx:328,329` | 事件 | 已登记 `ADR-0001#start-focus-hover` | 键盘焦点 = 悬停 |
| `web/src/start/StartPanel.tsx:332` | 事件 | 已登记 `ADR-0001#start-button-hitbox-dom` | 只收键盘的激活（`detail` 为 0）：原版标题页没有键盘输入，这一条是「真 `<button>`、回车按得动」那一半（真浏览器里曾经按不动，`xl-fqm` 修了）。鼠标不走它 —— 按下 / 松手分两下送（xl-4zo） |
| `web/src/start/StartPanel.tsx:344,353,368,374,384,410,419,435` | 无障碍 / 元素 | 工程 | 装饰图的 `alt=""`（读屏跳过） |
| `web/src/start/StartPanel.tsx:360` | 元素 / 事件 | 对应 `src/start/StartPanel.java:185` | `mousePressed`：面板上任何一处按下，先记自绘鼠标坐标（头一句 `currentX = e.getX()`，xl-40m）（落在空处按 `null`，只把图换回常态）；grab 不在这块面板上而这一下之外还按着别的键（舞台外按着键拖进来），`isMouseGrab` 为真、目标仍是 null，不收；收下的每一只键都记进那张位图，上面那一行的松手照它判该不该送（xl-bg3）（xl-m9q；全松开那一下 JDK 本会派松手，而 macOS 上实测它根本到不了 Java 窗口、原版也不收 —— **没有残余差异**。读数取自 CGEvent 合成的序列，窗口外那一下按在另一个 app 的窗口上，xl-zs6；复跑 `tools/mouse-dispatch-probe.sh`，xl-sij；换成**同一个 app 的另一块窗口**（xl-g9w）或**露出来的桌面**（xl-23v）结论都相同）；同一行的 `onMouseMove` 是自绘鼠标的坐标（`mouseMoved` 头一句 `currentX = ex.getX()`；按着键是 `mouseDragged`（`:211`），归挂到 window 上的那个 `mousemove`（`:213`），只在 grab 挂在这块面板上时才记 —— 舞台外按下再拖进来，原版目标是 null、一次都不派（xl-b98））；同一行的 `onDragStart` 取消面板上图片的原生拖放 —— 不取消的话真 Chrome 里在背景上按下一拖就 `pointercancel`，之后的 `mouseDragged` / `mouseReleased` 一个都收不到（xl-qzx） |
