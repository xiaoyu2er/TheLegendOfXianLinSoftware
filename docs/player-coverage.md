...
**现查：2026-09-11，xl-03x.2（open）。共 61 行（能 41 · 不能 9 · 能但不对 11）。**
...
- 依据列的标法：**实跑** = 本票现查时实际跑过（测试条数是那一次的读数）；**读代码** = 没跑过，是从源码读出来的。
...
- **快照不是 `.beads/issues.jsonl`**：那是 bd 的被动导出，现查时是过期的（125 条、open 25；活库 251 条、非 closed 84）。
...
- **只立票的**（已立，都在上表第四列）：
  ...
  | 票 | 为什么过不了线 |
  |---|---|
  | xl-19z（open）战斗背景音乐 10 首抛 | 逐帧比对不放声音；判据该落在单元缝上 |
...
1. **范围没有被撑大，但有一张可能要出去**：xl-03x.16（open）（Esc 那两道门）进这一轮的理由是「写条剧本就能看见」，
   现查（读代码，未跑）门禁在会话层，而取图页只调 `step()`、不走会话层 —— 一条普通 scene 剧本按 Esc，两端都停在场景上。
   已评论在票上。
2. **xl-19z（open）是这一轮现查出来最重的一条**：玩家进大多数战斗都会撞上。按线它不进这一轮，要不要破例由主干定。
3. **「结」口径冲突**：xl-u23（closed）的裁定（禁用 + title）已落地，xl-03x.12（open）要的是「画出来、点了什么都不发生」。
4. **重复票**（没动，列出来）：xl-2d5（open）≈ xl-03x.14（open）· xl-as2（open）≈ xl-03x.16（open）·
