...
import { EXPECTED } from '../../compare/expected'
...
describe('battleTextureIds 推的名单，盖得住每一条真值真的画到的每一张图', () => {
  ...
  const drawn = BATTLE_TRACE_NAMES.map((name) => {
    ...
    // **撞上一层还没实现的绘制，就停在那一拍并把话记下来**（xl-rh9.18）。
    //
    // 原先这里是一路画到底的：那时 `battle-mishu-lu` 还归 xl-rh9.12 的那批
    // unpainted 里，收集阶段就当场抛，整个文件连收集都跑不完（vitest 报的是
    // `FAIL … [ assets.test.ts ]`，一条 `it` 都没跑）。
    //
    // ⚠️ **不能简单 `try {} catch {}` 跳过** —— 那等于让这条判据对那份真值
    // 恒过：真值再多几份画不出来的，它一声不吭。所以停下来是**记账**：
    // `stoppedAt` / `stopMessage` 会被下面那条「停下来的正是表上登记的那几条」
    // 撞一遍，多一条少一条都红。
    ...
    for (const tick of trace.ticks) {
      ...
      try {
        ...
        stoppedAt = i
        stopMessage = e instanceof Error ? e.message : String(e)
        break
      }
      for (const op of ops) {
    ...
    return { name, loaded, used, stoppedAt, stopMessage }
  })
  ...
  it('中途停下来的，正是 expected.ts 里表 unpainted 的那几条（xl-rh9.18）', () => {
    // 双向对撞，两头都会红：
    //
    // - 某条剧本撞上没实现的层却**没有**表 unpainted → 它出现在左边、不在右边；
    // - 表着 unpainted 却**一路画到底** → 反过来。
    //
    // 右边这份名单是**登记**（`EXPECTED` 里手写的表态），不是现扫出来的，
    // 所以它撞得动左边。
    const stopped = drawn.filter((d) => d.stoppedAt !== null).map((d) => d.name)
    const declared = BATTLE_TRACE_NAMES.filter((n) => EXPECTED[n]?.status === 'unpainted')
    expect(stopped.sort()).toEqual([...declared].sort())
  })

  it('停下来的那一句话，点的是表上挂的那张票', () => {
    // 「画不出来」与「画错了炸了」在报告里长得一样，所以要核那句话。
    for (const d of drawn) {
      if (d.stoppedAt === null) continue
      const issue = EXPECTED[d.name]?.issue
      expect(issue, `${d.name} 表 unpainted 却没挂票号`).toBeTruthy()
      expect(d.stopMessage, `${d.name} 第 ${d.stoppedAt} 拍抛的这句没点名 ${issue}`).toContain(
        issue!,
      )
    }
  })
